...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image = pygame.transform.scale(player_img, (48, 38))
    ...
    def update(self):
        ...
        if self.power == 2 and pygame.time.get_ticks() - self.power_time > self.power_duration:
            self.power = 1
            self.power_time = pygame.time.get_ticks()
    ...
    def hide(self):
        self.hidden = True
        self.hide_timer = pygame.time.get_ticks()
        self.rect.center = (WIDTH + 100, HEIGHT + 200)
...
class Mob(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rot = 0
        self.rot_speed = random.randrange(-12, 14)
...
def show_go_screen(score):
    ...
    if score >= 0:
        draw_text(screen, "Your Score is " + str(score), 20, WIDTH / 2, (HEIGHT / 4) + 85, YELLOW)
    draw_text(screen, "Arrow Keys - Move", 22, WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Space - Fire", 22, WIDTH / 2, (HEIGHT / 2)+30)
    draw_text(screen, "Press any key to Begin", 18, WIDTH / 2, (3 * HEIGHT) / 4)
    pygame.display.flip()
...
meteor_images = []
meteor_list = ['spaceMeteors_004.png', 'meteorBrown_med1.png', 'meteorBrown_med3.png',
               'meteorBrown_small1.png', 'meteorBrown_small2.png', 'meteorBrown_tiny1.png']
for img in meteor_list:
    if img == 'spaceMeteors_004.png':
        meteor_images.append(pygame.transform.scale(pygame.image.load(os.path.join(img_dir, img)).convert(), (60, 65)))
    else:
        meteor_images.append(pygame.image.load(os.path.join(img_dir, img)).convert())

p_ups = {'shield': pygame.image.load(os.path.join(img_dir, "shield_gold.png")),
         'gun': pygame.image.load(os.path.join(img_dir, "bolt_gold.png"))}

explosion_anim = {'lg': [], 'sm': [], 'player': []}
for i in range(9):
    filename = 'hit_000{}.png'.format(i)
...
for i in range(31):
    if i < 10:
        filename = 'expl_06_000{}.png'.format(i)
    else:
        filename = 'expl_06_00{}.png'.format(i)
    img = pygame.image.load(os.path.join(img_dir, filename)).convert()
    img.set_colorkey(BLACK)
    explosion_anim['player'].append(img)

sound_shoot = pygame.mixer.Sound(os.path.join(snd_dir, "sfx_laser2.ogg"))

shield_sound = pygame.mixer.Sound(os.path.join(snd_dir, "Powerup3.wav"))
gun_sound = pygame.mixer.Sound(os.path.join(snd_dir, "Powerup_gun.wav"))

expl_sounds = []
for snd in ['Explosion.wav', 'Explosion7.wav']:
    expl_sounds.append(pygame.mixer.Sound(os.path.join(snd_dir, snd)))

player_die_sound = pygame.mixer.Sound(os.path.join(snd_dir, "Explosion11.wav"))
...
while running:
    ...
    draw_text(screen, "SCORE: " + str(score), 18, WIDTH/2, 0)
